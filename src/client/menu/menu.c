...
PlayerDirectoryList(void)
{
	...
	for (i = 0; i < num; ++i)
	{
		 ...
		 * directories
		 */
		slash = strchr(list.data[i] + listoff, '/');
		if (slash)
		{
			*slash = '\0';
		}
...
strlist_t
AllSkinsInDir(const char *dirname)
{
	strlist_t list;

	StrList_Init(&list, 0);

	SkinsInDir(&list, dirname, "pcx");
	SkinsInDir(&list, dirname, "png");
...
PlayerModelList(void)
{
	...
	for (i = 0; i < s_directory.num; ++i)
	{
		...
		if (!s || !ContainsFile(s, "tris.md2"))
		{
			continue;
		}

		list = AllSkinsInDir(s_directory.data[i]);
		if (!list.num)
		{
			continue;
		}
		...
		for (k = 0; k < list.num; ++k)
		{
			...
			ReplaceCharacters(list.data[k], '\\', '/');

			t = strrchr(list.data[k], '/');
			if (!t)
			{
				t = list.data[k];
			}
